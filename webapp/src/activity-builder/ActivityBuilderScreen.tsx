...
import ActivityReviewScreen from './ActivityReviewScreen';
...
import type { ActivityDiagramState, ActivityLesson, FeedbackItem } from './engine/types';
import { useMistakesStore } from '../store/mistakes';

interface ActivityBuilderScreenProps {
  lesson: ActivityLesson;
  initialDiagram?: ActivityDiagramState;
  initialStepIndex?: number;
  onProgress?: (diagram: ActivityDiagramState, stepIndex: number) => void;
  onComplete?: () => void;
}

function emptyDiagram(): ActivityDiagramState {
  return { nodes: [], edges: [] };
}
...
export default function ActivityBuilderScreen({
  ...
  onProgress,
  onComplete,
}: ActivityBuilderScreenProps) {
  const [diagram, setDiagram] = useState<ActivityDiagramState>(initialDiagram ?? emptyDiagram());
  ...
  // quyết") — the learner decides when they're done. Guided's 'compare' step keeps
  // requiring a clean diff before "Hoàn thành", since guided mode's whole premise is
  // steering the learner to full correctness.
